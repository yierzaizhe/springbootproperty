...
/**
 * @author ylz
 * @Description 保在一次请求只通过一次filter，而不需要重复执行
 * @date 2021-03-17-22:50
 */
@Component
@Slf4j
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            if (redisUtil.isBlackList(authToken)) {
                ...
                response.getWriter().write(JSON.toJSONString(ResultTool.fail(ResultCode.TOKEN_IS_BLACKLIST)));
                return;
            }
            ...
            if (redisUtil.hasKey(authToken)) {//判断redis是否有保存
                ...
                if (JwtTokenUtil.isExpiration(expirationTime)) {
                    ...
                    if (DateUtil.compareDate(currentTime, tokenValidTime)) {
                        ...
                        response.getWriter().write(JSON.toJSONString(ResultTool.fail(ResultCode.LOGIN_IS_OVERDUE)));
                        return;
